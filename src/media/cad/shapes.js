...
function fshapes(svg, setDrawing, width, height) {
    ...
    this.displayDate = function (evt) {
        ...
        evt.addEventListener('mousedown', startDrag);
        evt.addEventListener('mousemove', drag);
        evt.addEventListener('mouseup', endDrag);
        evt.addEventListener('mouseleave', endDrag);
      ...
      //modifedSizeDivByoscilloscope(newElement);
        ...
        case 'ref':
          ...
          //  newElement.setAttribute("alignment-baseline","middle");
            ...
            if(element=='param')
            {
                newElement.setAttribute("paramname", "p");
                newElement.setAttribute("paramvalue", "100");
                newElement.setAttribute("show", "Name and value");
            }
    ...
    self.getPosCriticalElement = function (evtTarget) {
     ...
     //****************************************************end polyline******************************************************************//
      ...
      var els = document.getElementsByClassName("codeSpice");
        for (var i = els.length-1; i >=0 ; i--)
                if (pointInRect(els[i], self.offset)) {
                      self.setCritElem = els[i];
                      return true;
